...
Met `@Output()` kan een childcomponent een event "uitsturen" naar de parent. Zo kan het child bijvoorbeeld een actie melden of data terugsturen wanneer er iets gebeurt.
```ts
// src/app/profiel/profiel.ts
...
We kunnen route parameters uitlezen via de `ActivatedRoute`. Elke keer als de parameter verandert (bijvoorbeeld wanneer de gebruiker navigeert naar een ander profiel) zal de `paramMap`-observable een nieuwe waarde doorgeven.
```ts
// src/app/profiel/profiel.ts
import { Component } from '@angular/core';
...
<Tabs>
  ...
  export const unsavedChangesGuard: CanDeactivateFn<unknown> = (component, route, state) => {
